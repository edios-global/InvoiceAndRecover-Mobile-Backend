...
const fetchAgedPayables = asyncHandler(async (req, res) => {
    try {
        ...
        const fetchList = [
            {
                $match: {
                    $and: [
                        { businessUserID: mongoose.Types.ObjectId(post.businessUserID), rctiStatus: "Unpaid" },
                        {
        ...
        let myItemAggregation = RCTI.aggregate(fetchList);
        RCTI.aggregatePaginate(
            ...
            (err, result) => {
                if (err) {
                    ...
                    const successResponse = genericResponse(true, "fetchAgedPayables fetched successfully.", { result });
...
export default fetchAgedPayables;
